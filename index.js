...
const clientstart = async () => {
    ...
    sock.ev.on('messages.update', async updates => {
        ...
        const ownerJid = (cfg().owner || '').replace(/[^0-9]/g, '') + '@s.whatsapp.net';
        ...
        for (const { key, update } of updates) {
            ...
            const tgt = adTarget === 'same' ? key.remoteJid : ownerJid;
            ...
            try {
                if (msgType === 'conversation' || msgType === 'extendedTextMessage') {
                    ...
                    await sock.sendMessage(tgt, { text: `${hdr}\n\n💬 "${txt}"` }).catch(() => {});
                } else if (msgType === 'imageMessage') {
                    const buf = await sock.downloadMediaMessage(del).catch(() => null);
                    if (buf) await sock.sendMessage(tgt, { image: buf, caption: hdr }).catch(() => {});
                    else await sock.sendMessage(tgt, { text: hdr + '\n🖼️ [Image]' }).catch(() => {});
                } else {
                    await sock.sendMessage(tgt, { text: hdr + `\n[${msgType}]` }).catch(() => {});
                }
            } catch(_) {}
